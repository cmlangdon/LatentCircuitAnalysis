...
def generate_input_target_stream(context, motion_coh, color_coh, baseline, alpha, sigma_in, n_t, cue_on, cue_off,
    ...
    motion_input[stim_on - 1:stim_off, 2] = motion_r * np.ones([stim_off - stim_on + 1])
    motion_input[stim_on - 1:stim_off, 3] = motion_l * np.ones([stim_off - stim_on + 1])

    # Color input stream
    color_input = np.zeros([n_t, 6])
    color_input[stim_on - 1:stim_off, 4] = color_r * np.ones([stim_off - stim_on + 1])
    color_input[stim_on - 1:stim_off, 5] = color_l * np.ones([stim_off - stim_on + 1])
...
def generate_trials( n_trials, alpha, tau, sigma_in, baseline, n_coh, n_t, cue_on, cue_off,
                                 stim_on,stim_off, dec_on, dec_off):
    """
